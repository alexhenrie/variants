...
sub M_genomes {
	...
	foreach my $elements (@file) {
		...
		my ($chromosome, $source, $type, $start, $end, $length, 
		$strand, $col, $remainder) = split(/\t/, $elements);	

		$remainder =~ /Reference_seq=([^\;]+)\;/;
		my $ref_allele = $1;

		$remainder =~ /Variant_seq=([^\;]+)\;/;
		my $var_allele = $1;	

		$remainder =~ /Variant_freq=([^\;]+)\;/;
		my $var_stat = $1;
		my $ref_stat = 1 - $var_stat;
	
		my $location = join(':', $chromosome, $start );
